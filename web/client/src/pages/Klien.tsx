...
export default function KlienPage() {
  ...
  const { items: kliens, status } = useAppSelector((s) => s.klien)
  ...
  const load = useCallback(() => { dispatch(fetchKliens()) }, [dispatch])
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editItem) await dispatch(updateKlien({ id: editItem.id, payload })).unwrap()
      else await dispatch(createKlien(payload)).unwrap()
      setShowForm(false)
      setEditItem(null)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Gagal menyimpan klien')
    }
  }
  ...
  const handleDelete = async () => {
    if (!confirm) return
    try {
      await dispatch(deleteKlien(confirm.id)).unwrap()
    } catch (err) {
  ...
  if (status === 'loading' && kliens.length === 0) return <div className="page-loading"><div className="spinner" /><span>Memuat klien...</span></div>
