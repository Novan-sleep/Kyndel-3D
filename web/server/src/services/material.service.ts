...
export const materialService = {
  ...
  create(payload: any) {
    if (!payload.nama?.trim()) throw new HttpError(400, 'Nama material wajib diisi')
    if (payload.stok < 0) throw new HttpError(400, 'Stok tidak boleh negatif')
    if (payload.hargaBeliPerGram <= 0) throw new HttpError(400, 'Harga beli per gram harus lebih dari 0')
    if (payload.hargaJualPerGram <= 0) throw new HttpError(400, 'Harga jual per gram harus lebih dari 0')
    if (payload.hargaJualPerGram < payload.hargaBeliPerGram) throw new HttpError(400, 'Harga jual tidak boleh lebih kecil dari harga beli')
  ...
  update(id: string, payload: any) {
    ...
    if (payload.stok !== undefined && payload.stok < 0) throw new HttpError(400, 'Stok tidak boleh negatif')
    if (payload.hargaBeliPerGram !== undefined && payload.hargaBeliPerGram <= 0) throw new HttpError(400, 'Harga beli harus lebih dari 0')
    if (payload.hargaJualPerGram !== undefined && payload.hargaJualPerGram <= 0) throw new HttpError(400, 'Harga jual harus lebih dari 0')
    ...
    if (finalJual < finalBeli) throw new HttpError(400, 'Harga jual tidak boleh lebih kecil dari harga beli')
  ...
  restock(id: string, jumlahKg: number, hargaBeliPerGram: number, catatan?: string) {
    if (jumlahKg <= 0) throw new HttpError(400, 'Jumlah restock harus lebih dari 0')
    if (hargaBeliPerGram <= 0) throw new HttpError(400, 'Harga beli harus lebih dari 0')
    const material = materialRepository.findById(id)
    if (!material) throw new HttpError(404, 'Material tidak ditemukan')
    if (hargaBeliPerGram > material.hargaJualPerGram) throw new HttpError(400, 'Harga beli tidak boleh lebih besar dari harga jual saat ini')
  ...
  delete(id: string) {
    const material = materialRepository.findById(id)
    if (!material) throw new HttpError(404, 'Material tidak ditemukan')
    if (pesananRepository.findAktif().some(p => p.materialId === id)) throw new HttpError(400, 'Material masih terkait dengan pesanan aktif')
