...
const WITH_STATS = `
  SELECT k.*,
    ...
    COALESCE(SUM(CASE WHEN p.status = 'Selesai' THEN
      p.nilai_jual - CASE
        WHEN p.diskon_tipe = 'persen' THEN ROUND(p.nilai_jual * p.diskon_nilai / 100)
        WHEN p.diskon_tipe = 'rupiah' THEN p.diskon_nilai
        ELSE 0
      END
    ELSE 0 END), 0) AS total_nilai
  FROM klien k
