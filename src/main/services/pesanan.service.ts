...
export const pesananService = {
  ...
  getById(id: string) { return pesananRepository.findById(id) },
  create(payload: any) {
    const printer = printerRepository.findById(payload.printerId)
    if (!printer) throw new Error("Printer tidak ditemukan")
    if (printer.status !== "Idle") throw new Error(`Printer "${printer.nama}" sedang tidak Idle`)
    ...
    if (payload.multiColorData && payload.multiColorData.length > 0) {
      const resolved = payload.multiColorData.map((entry: any) => {
        ...
        if (mat.stok < entry.beratGram / 1000) throw new Error(`Stok "${mat.nama}" tidak cukup. Tersedia: ${mat.stok} kg, dibutuhkan: ${(entry.beratGram / 1000).toFixed(3)} kg`)
      ...
      if (material.stok < payload.beratMaterial) throw new Error(`Stok material tidak cukup. Tersedia: ${material.stok} kg`)
  ...
  update(id: string, payload: any) {
    ...
    if (existing.status !== "Antrian") throw new Error("Hanya pesanan Antrian yang dapat diubah")
  ...
  mulaiPrinting(id: string) {
    return withTransaction(() => {
      ...
      if (pesanan.status !== "Antrian") throw new Error("Hanya pesanan Antrian yang bisa dimulai")
  ...
  selesaikan(id: string) {
    return withTransaction(() => {
      const pesanan = pesananRepository.findById(id)
      if (!pesanan) throw new Error("Pesanan tidak ditemukan")
      if (pesanan.status !== "Printing") throw new Error("Hanya pesanan Printing yang bisa diselesaikan")
      const now = nowIso()
  ...
  batalkan(id: string) {
    return withTransaction(() => {
      const pesanan = pesananRepository.findById(id)
      if (!pesanan) throw new Error("Pesanan tidak ditemukan")
      if (!["Antrian", "Printing"].includes(pesanan.status)) throw new Error("Pesanan ini tidak bisa dibatalkan")
      const now = nowIso()
  ...
  hapus(id: string) {
    const pesanan = pesananRepository.findById(id)
    if (!pesanan) throw new Error("Pesanan tidak ditemukan")
    if (!["Selesai", "Dibatalkan"].includes(pesanan.status)) throw new Error("Hanya pesanan Selesai atau Dibatalkan yang dapat dihapus")
