...
export const aktivitasRepository = {
  ...
  findByModul(modul: AktivitasModul, limit = 50): Aktivitas[] { return getDb().prepare('SELECT * FROM aktivitas WHERE modul = ? ORDER BY created_at DESC LIMIT ?').all(modul, limit).map(rowToAktivitas) },
