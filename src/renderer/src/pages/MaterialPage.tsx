...
export default function MaterialPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const res = await api.material.getAll()
  ...
  const handleRestock = async () => {
    ...
    const res = await api.material.restock(
      restockItem.id,
      +restockForm.jumlahKg,
      +restockForm.hargaBeliPerGram,
      restockForm.catatan.trim() || undefined
    )
    if (res.success) { setRestockItem(null); load() }
    else setRestockError(res.error)
  }
  ...
  const handleSubmit = async () => {
    ...
    const res = editItem ? await api.material.update(editItem.id, payload) : await api.material.create(payload)
    if (res.success) { setShowForm(false); setEditItem(null); load() }
    else setError(res.error)
  }

  if (loading) return <div className="page-loading"><div className="spinner" /><span>Memuat material...</span></div>

  return (
    <div>
      <div className="animate-fade-up" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--spacing-lg)' }}>
        <div style={{ position: 'relative' }}>
          ...
          <input
            ...
            style={{ paddingLeft: '28px', paddingRight: search ? '28px' : undefined, width: '220px' }}
          />
          {search && (
        ...
        <div className="modal-overlay">
          <div className="modal-box" style={{ width: '440px' }}>
            ...
            <div className="modal-body">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: 'var(--spacing-lg)' }}>
      ...
      {confirm && <ConfirmDialog title="Hapus Material" message={`Hapus material "${confirm.nama}"?`} onConfirm={async () => { await api.material.delete(confirm.id); setConfirm(null); load() }} onCancel={() => setConfirm(null)} confirmLabel="Hapus" danger />}
    </div>
  )
}
