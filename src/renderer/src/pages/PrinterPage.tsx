...
export default function PrinterPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    const res = await api.printer.getAll()
  ...
  const handleSubmit = async () => {
    setError('')
    const payload = { nama: form.nama, model: form.model || undefined, tipe: form.tipe || undefined, watt: +form.watt, catatan: form.catatan || undefined }
    const res = editItem ? await api.printer.update(editItem.id, payload) : await api.printer.create(payload)
    if (res.success) { setShowForm(false); setEditItem(null); load() }
    else setError(res.error)
  }

  const handleSetStatus = async (id: string, status: 'Idle' | 'Maintenance' | 'Rusak') => {
    setStatusMenu(null)
    const res = await api.printer.setStatusManual(id, status)
  ...
  const handleDelete = async () => {
    if (!confirm) return
    const res = await api.printer.delete(confirm.id)
  ...
  if (loading) return <div className="page-loading"><div className="spinner" /><span>Memuat printer...</span></div>
