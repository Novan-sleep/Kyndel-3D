...
import { Pesanan, Material, Printer } from '../../types'

const pageTitles: Record<Page, string> = {
  dashboard: 'Dashboard', pesanan: 'Manajemen Pesanan', printer: 'Manajemen Printer',
  material: 'Manajemen Material', keuangan: 'Keuangan', aktivitas: 'Log Aktivitas', setting: 'Pengaturan',
}
...
export default function Topbar({ currentPage, theme, onToggleTheme, onNavigate }: Props) {
  ...
  async function fetchNotifs() {
    ...
    try {
      const [kpiRes, printerRes] = await Promise.all([
        window.api.dashboard.getKPI(),
        window.api.printer.getAll()
      ])
      const list: Notif[] = []

      if (kpiRes.success) {
        for (const p of (kpiRes.data.orderMendekatiDeadline as Pesanan[]) || []) {
        ...
        for (const m of (kpiRes.data.materialHampirHabis as Material[]) || []) {
          list.push({
            type: 'stok',
      ...
      if (printerRes.success) {
        for (const p of (printerRes.data as Printer[])) {
      ...
      try {
        if (!cacheRef.current) {
          const [pr, mr, tr] = await Promise.all([
            window.api.pesanan.getAll(), window.api.material.getAll(), window.api.printer.getAll()
          ])
          cacheRef.current = {
            pesanan: pr.success ? pr.data : [],
            material: mr.success ? mr.data : [],
            printer: tr.success ? tr.data : []
          }
        }
  ...
  return (
    <header style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
        <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>Pages</span>
        <span style={{ fontSize: '11px', color: 'var(--border-subtle)', opacity: 0.6 }}>/</span>
